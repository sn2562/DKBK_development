...
public class Tool extends Button {//ツールバー,ボタンをextendsしてるのはmouseOverを楽にとるため
  ...
  public Tool() {
    ...
    for (int i=0; i<toolButton.length; i++) {
      ...
      if (i==4) {
        ...
        toolButton[i]=new ImButton(g, 6, 50*(i-3)+Y, 48, 48);
      } else {
    ...
    movButton=new Im2Button(loadImage("icon-mov0.png"), loadImage("icon-mov1.png"), 6, Y, 48, 24);

    Y=Y+26+13;
    resetMoveButton=new ImButton(loadImage("icon-resetmov.png"), 6, Y, 48, 24);

    Y=Y+26+13;
    animButton=new ImButton(loadImage("icon-resetmov.png"), 6, Y, 48, 24);
  ...
  public void update() {
    ...
    //    if (chgButton.isPressed) {
    //      println("カメラ-キャンバスの切り替え");
    //      chgButton.setSelected(false);
    //      chgButton.no=!chgButton.no;
    //    }
  ...
  private void buttonSelect(ImButton m[], int n) {//n番目のボタンを選択しn番目以外のボタンの選択を解除する
    ...
    println("tool.nowToolNumber :"+tool.nowToolNumber);

    if (oldToolNumber==tool.nowToolNumber&&mouseY>200) {//もし複数回クリックならば
    ...
    if (nowToolNumber==0) {//ペンモードの時の移動
      //penMode=true;//本来ならtrue
      println("青ペン");
    } else if (nowToolNumber==1) {//青ペンモードの時の移動
      penMode=false;
      println("青ペン");
      data.get(tool.nowDataNumber).matrixReset();
    } else {//それ以外は何もしない
