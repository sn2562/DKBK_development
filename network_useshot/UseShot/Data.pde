...
public class Data {//DepthDatadrawを並列処理にすれば軽くなるか？
  ...
  private ArrayList<PVector> doublelines;//二重補正用
  ...
  private void ready() {
    ...
    draw_mode=0;
    matrixReset();
    doublelines=new ArrayList<PVector>();
  }
  ...
  private void setMatrix2() {//depthの初期位置設定
    tool.setMatrix();
    translate(width/2, height/2);
    rotateX(PI);
    rotateY(0);
  ...
  public void update() {
    ...
    if (penMode) {//ペンモードなら回転
      //todo
      //もし、回転操作をしていたら新しいDepthは位置を再計算して表示する
      if (!(pos.x==0&&pos.y==0&&pos.z==0&&abs(sin(rotX-PI))<0.01&&abs(sin(rotY-PI))<0.01)) {
        // println("再計算じゃ");
        setMatrix();
      } else {
        //回転していないなら古いデプスの位置のままで
        //println("そのままの位置じゃ");
        setMatrix();
      }
    } else
  ...
  public PVector PCanvas(int x, int y) {
    ...
    try {//たまにArrayIndexOutOfBoundsExceptionでるのでその例外をはじいておく。
      ...
      if (line.size()>0&&tool.nowToolNumber==0) {//線を描いていたら,かつ補正ペンなら
        ...
        float ay=pp.y+pos.y;
        float az=pp.z+pos.z;
        //todo:test用 右クリック中(直線を描く)時は補正をかけない
        if (new PVector(ax, ay, az).dist(p)>tool.getBorder()&&mouseButton != RIGHT) {//距離が大きすぎるようなら前の点の座標のを利用(補正する)
  ...
  private void autosave() {
    ...
    try {
      ...
      FileDialog fd=new FileDialog(frame, "ファイルを選択してください", FileDialog.SAVE);
       ...
       fd.setVisible(true);
       */
      String path="/Users/kawasemi/Dropbox/intraction_mac/"+hour()+"_"+minute()+"_"+second()+".dsd";//mac版/選択したファイル
      //String path="C:\\Users\\sumi_000\\Desktop\\Dropbox\\kikuchi"+hour()+"_"+minute()+"_"+second()+".dsd";//windows版
      path="/Users/kawasemi/Desktop/dsdData/"+hour()+"_"+minute()+"_"+second()+".dsd";//mac版/選択したファイル
  ...
  private void saveSketch() {
    ...
    try {
      ...
      FileDialog fd=new FileDialog(frame, "ファイルを選択してください", FileDialog.SAVE);
       ...
       fd.setVisible(true);
       */
      String path="/Users/kawasemi/Dropbox/intraction_mac/"+hour()+"_"+minute()+"_"+second()+"_sketch.dsd";//選択したファイル
      //String path="C:\\Users\\sumi_000\\Desktop\\Dropbox\\kikuchi"+hour()+"_"+minute()+"_"+second()+".dsd";//windows版
      path="/Users/kawasemi/Desktop/dsdData/"+hour()+"_"+minute()+"_"+second()+".dsd";//mac版/選択したファイル
